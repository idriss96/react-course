const data = [
  ...
  },
  {
    ...
    status: 'PUBLISH',
    authors: ['Florian Müller', 'Jay Brown', 'Jeff Potts', 'idriss atik'],
    categories: [],
  },
  {
    _id: 667,
...
const { title, authors, pageCount, categories, shortDescription } = book;

// const primaryGener = authors[0];
// const secondaryGener = authors[1];

// select the first and second element from the array
// const [primaryGener, secondaryGener] = authors;

// select the first and second and other elements from the array
const [primaryGener, secondaryGener, ...otherGener] = authors;

// add new auther to the authers array
const newAuther = [...authors, 'add auther'];

// add new property to the object
const newBook = { ...book, categories: ['new property'] };

const summary = `${title} by ${primaryGener}`;

const pagesLimmet = pageCount > 100 ? 'long book' : 'short book';

// function
// function getTitle(id) {
//   return data.find((book) => book.id === id).title;
// }
...
// map method
const booksTitles = data.map((book) => book.title);

const essentiaslData = data.map((book) => {
  return {
    title: book.title,
...
// reduce method
const totalPageCount = data.reduce((acc, book) => acc + book.pageCount, 0);

// sort method
const sortedBooks = data.sort((a, b) => a.pageCount - b.pageCount);
